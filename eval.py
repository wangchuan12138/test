...
# 对点云数据集进行划分建立kd树，这样有利于后续对树的临近点进行检索，提高效率
def kdtree_partition(pc, max_num):
    ...
    print("the parts len are :",len(parts))

    return parts
# 对点云数据集进行分割，分割为几何信息和颜色信息
def partition_point_cloud(filedir, max_num=300000):
    ...
    for i, pc_part in enumerate(parts):
        ...
        #os.path.split(filedir)返回一个数组，包含一个是路径名，还有一个文件名
        #例如：filedir_part is : ('testdata/8iVFB', 'redandblack_vox10_1550.ply')
        #os.path.aplit(filedir)[-1]指的是倒数第一个元素 就是文件名，亦可使用[1]来代替
        #后面的[:-4]指的是取到倒数第四个元素为止
...
def sort_sparse_tensor(coords, feats):
    ...
    coords = coords.long()
    #print("coords.long is :",coords.long)
    minimum = coords.min()
    #print("coords.min is :",coords.min)
    coords = coords - minimum
    #print("coords processed is :",coords)
    step = coords.max() + 1
    #print("step is :",step)
    coords_sum = coords[:, 1] + coords[:, 2]*step + coords[:, 3]*step*step
    #print("coords_sum is :",coords_sum)
...
def encode(filedir, pcc, prefix, voxel_size):
    ...
    with torch.no_grad():
        ys = pcc.encoder(x)
        print("out1 is :", ys[1])
        print("out1 输出特征是 ： ", ys[1].features.size())
...
def decode(y_coords_binname, y_feats_binname, head_binname, pcc, rho, voxel_size):
    ...
    # print("original y_coords is :",y_coords)
    tensor_stride = 8
    sort_xyz_coordinates = sort_xyz(y_coords)
    # print("sorted coordinates is :",sort_xyz_coordinates)
    y_coords = sort_xyz(y_coords)*tensor_stride
    # print("coordinates after multiply is :",y_coords)
...
def eval(filedir, csv_root_dir, ckptdirs, voxel_sizes, rhos, res, max_num):
    ...
    for idx_ckpt, ckptdir in enumerate(ckptdirs):
        ...
        if os.path.exists(ckptdir):
            # 加载检查点的状态 包括encoder decoder entropy_bottleneck
